...
def get_stdev(nums):
    ...
    mean = get_avg(nums)
    n = len(nums)
    res = math.sqrt(sum([(x-mean)**2 for x in nums])/(n - 1))
    return res 
...
def get_standard_error(a, b):
    ...
    stda = get_stdev(a)
    stdb = get_stdev(b)
    return math.sqrt((stda**2)/len(a)+(stdb**2)/len(b))
...
def col_sum(observed_grid, ele_col):
    return sum([row[ele_col] for row in observed_grid])

def total_sum(observed_grid):
    return sum([row_sum(observed_grid, i) for i in range(len(observed_grid))])
...
def get_expected_grid(observed_grid):
    ...
    ** DO NOT modify the parameter, observed_grid. **
    ...
    row_sums = [row_sum(observed_grid, i) for i in range(rows)]
    col_sums = [col_sum(observed_grid, j) for j in range(cols)]
    ...
    for i in range(rows):
        ...
        for j in range(cols):
            ...
            expected = calculate_expected(row_s, col_s, tot_sum)
            row.append(expected)
        ret.append(row)
    return ret
...
def chi2_value(observed_grid):
    ...
    for i in range(rows):
        for j in range(cols):
           ret += ((observed_grid[i][j] - expected_grid[i][j])**2) / expected_grid[i][j]
